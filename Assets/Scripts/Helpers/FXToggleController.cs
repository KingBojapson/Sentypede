...
public class FXToggleController : MonoBehaviour 
{
	...
	void Awake()
	{
		...
		offPosX = onPosX * -1;
        if (SoundManager.soundInstance.musicVolume >= 1)
        {
            isOn = true;
        }
        else
	...
	public void DoYourStaff()
	{
        if (isOn) 
        {
            DataController.instance.fxPlay = 1;
        }
        else 
        {
            DataController.instance.fxPlay = 0;
        }
	}
